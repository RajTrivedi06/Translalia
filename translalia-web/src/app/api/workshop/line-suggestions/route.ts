...
export async function POST(req: Request) {
  ...
  try {
    ...
    const rateLimitKey = `suggestions:line:${user.id}:${today}`;
    ...
    if (process.env.DEBUG_SUGGESTIONS === "1") {
      console.log("[line-suggestions] Debug request:", {
        ...
        selectedVariant: parsed.selectedVariant ?? null,
      });
    }

    const cacheKey = `suggestions:line:${parsed.threadId}:${
      parsed.lineIndex
    ...
    console.error("[line-suggestions] Error:", error);
    return NextResponse.json(
