...
export default function LineCitationDrawer({
  ...
  async function saveProvenance() {
    ...
    const existing: any[] = Array.isArray(meta.provenance)
      ? meta.provenance
      : [];
    ...
    for (const m of mappings) {
      ...
      if (!found) {
        ...
        const dedupe = new Set(
          (found.sources || []).map((s: any) => `${s.version_id}:${s.line_idx}`)
        );
  ...
  return (
    <div className="fixed inset-0 z-50">
      ...
      />
      <div className="absolute right-0 top-0 h-full w-full max-w-[560px] bg-white dark:bg-neutral-900 shadow-xl p-4 overflow-y-auto">
        <div className="text-lg font-semibold">Per-line citation</div>

        <div className="mt-4 space-y-4">
          <div className="text-sm">Target line (in {node.display_label}):</div>
          <select
          ...
          <div className="flex justify-end">
            <button
              ...
              onClick={addMapping}
              disabled={
                targetIdx == null || !sourceVersionId || selectedSrc.size === 0
              }
            >
              Add mapping
            </button>
              ...
              <ul className="list-disc pl-5 text-sm space-y-1">
                {mappings.map((m, idx) => (
                  <li key={idx}>
                    ...
                    {m.line_idx_list.map((i) => i + 1).join(", ")} (from version{" "}
                    {shortId(m.version_id)})
                  </li>
                ))}
              </ul>
            </div>
